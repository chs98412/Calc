...
WebUI.initWidgets = function() {
    ...
    new WebUI.Container({
        desired_size: {width: 250, height: 80}, 
        horizontal_alignment: WebUI.Alignment.CENTER, 
        vertical_alignment: WebUI.Alignment.CENTER, 
        children: [ 
...
WebUI.Switch.prototype.initVisualItems = function() {
    ...
    if(this.is_pushed){
        arc1.set({"fill":this.fill_colorOn});
        arc2.set({"fill":this.fill_colorOn});
        rect.set({"fill":this.fill_colorOn});
    }

    else {
        arc1.set({"fill":this.fill_colorOff});
        arc2.set({"fill":this.fill_colorOff});
        rect.set({"fill":this.fill_colorOff});
    }
...
WebUI.Switch.prototype.handleMouseUp = function() {
    if (this.is_pushed) {
        console.log("asdf");
        if(!this.is_on){
        this.visual_items[3].animate('left', '+=50', { onChange: WebUI.canvas.renderAll.bind(WebUI.canvas) });
